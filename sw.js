...
const SW_VERSION = 'v3.2-fix';
const CACHE_NAME = 'prokat-cache-v4';
...
// ---- FETCH — ОФЛАЙН ПОДДЕРЖКА ----
self.addEventListener('fetch', event => {
    const url = new URL(event.request.url);

    // Пропускаем все внешние API — никогда не кешируем их запросы
    // ВАЖНО: script.google.com (Google Apps Script) должен быть здесь — иначе
    // SW вернёт кешированный старый токен вместо живого ответа
    if (url.hostname.includes('google.com')    ||   // script.google.com, *.google.com
        url.hostname.includes('googleapis.com') ||   // Google REST APIs
        url.hostname.includes('18gps')          ||   // 18gps.net команды и авторизация
        url.hostname.includes('firebase')       ||   // Firebase Auth / Firestore
        url.hostname.includes('firebaseio')     ||   // Firebase Realtime DB
        url.hostname.includes('gstatic.com')    ||   // Firebase JS SDK CDN
        url.hostname.includes('telegram.org')   ||   // Telegram Bot API
        url.hostname.includes('api.telegram')   ||
        url.hostname.includes('yandex')         ) {  // Yandex Maps API
        return; // не перехватываем — запрос идёт напрямую в сеть
    }
